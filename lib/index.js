...
function fuzzyMatch(matchKey, item) {
  ...
  case 'collection': return item && (Symbol.iterator in item || item.constructor.name === 'Object');
  }
}
 ...
 */
function composeNewAsyncFunction(originalFunction, funcName, logFunction) {
  return (...args) => {
    // Find the first "signature" that the arguments match.
    const functionConfig = functionConfigs[funcName].find(conf => {
